...
class TD3LandingAgent(Agent):

    def __init__(self, landing_area):
        super().__init__()
        self.landing_area = landing_area
        self.model = TD3.load(Settings.TD3_LANDER_BEST /
...
def train_landing_agent(init_env: Environment):
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=5000,
        save_path=Settings.TD3_LANDER_CHECKPOINT,
        name_prefix="td3_landing",
    ...
    eval_callback = EvalCallback(
        env,
        best_model_save_path=Settings.TD3_LANDER_BEST,
        eval_freq=5000,
        n_eval_episodes=100,
        deterministic=True
    )
